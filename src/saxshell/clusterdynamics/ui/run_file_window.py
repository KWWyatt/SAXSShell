...
class ClusterDynamicsRunFileWindow(QMainWindow):
    def __init__(
        self,
        *,
        ...
        self.setWindowTitle("Cluster Dynamics CLI Setup (Beta)")
    ...
    def _refresh_run_file_path(self) -> None:
        ...
        self.run_file_edit.setText(
            str(default_clusterdynamics_run_file_path(project_dir))
        )
    ...
    def _refresh_suggested_output_file(self) -> None:
        ...
        if project_dir is None or not frames_text:
            return
        try:
            suggested = suggest_clusterdynamics_output_file(
    ...
    def _save_run_file(self, *_args: object) -> None:
        try:
            ...
            QMessageBox.warning(self, "Cluster Dynamics CLI Setup", str(exc))
            return
        run_file_path = default_clusterdynamics_run_file_path(project_dir)
        save_clusterdynamics_run_config(run_file_path, config)
        ...
        QMessageBox.information(
            ...
            f"Saved cluster dynamics CLI run file:\n{run_file_path}",
        )
    ...
    def _update_preview(self, *_args: object) -> None:
        ...
        self._refresh_run_file_path()
        self.command_box.setPlainText(
            f'clusterdynamics run "{project_dir}"\n'
            f'saxshell clusterdynamics run "{project_dir}"'
        )
        try:
            config = self._current_config(project_dir)
    ...
    def _current_config(self, project_dir: Path):
        ...
        return build_clusterdynamics_run_config(
            project_dir=project_dir,
            frames_dir=frames_text,
...
def launch_clusterdynamics_run_file_ui(
    *,
    initial_project_dir: str | Path | None = None,
    initial_frames_dir: str | Path | None = None,
    initial_energy_file: str | Path | None = None,
